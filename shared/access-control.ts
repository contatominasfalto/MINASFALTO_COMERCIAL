...
export const ACCESS_CATALOG: AccessResource[] = [
  ...
  },
  {
    ...
    actions: [
      ...crud,
      { key: "import", label: "Importar", write: true },
      { key: "sync", label: "Sincronizar CRTI", write: true },
    ],
  },
  { key: "estoque", label: "Estoque", route: "/estoque", actions: crud },
  {
    ...
    actions: [
      ...crud,
      { key: "export", label: "Exportar", write: false },
      { key: "sync", label: "Sincronizar CRTI", write: true },
    ],
  },
  {
    ...
    actions: [
      ...crud,
      { key: "export", label: "Exportar relatórios", write: false },
      {
        key: "manage",
        label: "Gerenciar atas, adesões e cadastros",
        write: true,
      },
      { key: "documents", label: "Documentação da licitação", write: true },
      { key: "alerts", label: "Receber alertas de pregão?", write: false },
    ],
  },
  {
    key: "alimentacao",
    ...
    actions: [
      ...crud,
      { key: "export", label: "Exportar relatórios", write: false },
      { key: "manage", label: "Gerenciar cadastros", write: true },
    ],
  },
  {
    key: "compras",
    ...
    actions: [
      ...crud,
      { key: "export", label: "Exportar relatórios", write: false },
      { key: "import", label: "Importar planilhas", write: true },
      {
        key: "manage",
        label: "Gerenciar fornecedores e materiais",
        write: true,
      },
    ],
  },
  {
    ...
    actions: [
      ...crud,
      { key: "manage", label: "Alterar permissões", write: true },
    ],
  },
];
...
export function isMasterIdentity(
  user:
    | {
        username?: unknown;
        profile?: unknown;
  ...
  const identities = [user.username, user.profile, user.name, user.openId].map(
    normalizeIdentity
  );
  return identities.some(
    value =>
      value === MASTER_USERNAME || value === `local_login:${MASTER_USERNAME}`
  );
}
...
export function effectAllows(
  effect: PermissionEffect | null | undefined,
  action: string
) {
...
export function permissionTargetForProcedure(
  path: string,
  ...
  const resource =
    ...
    root === "indicadores"
      ? "comercial"
      : root === "pedidosObras" || root === "despesasTabelaGeral"
        ? "custo_obras"
        : root === "crti"
          ? path.toLowerCase().includes("obras") ||
            path.toLowerCase().includes("custos")
            ? "custo_obras"
            : "comercial"
          : root === "licitacoes"
            ? "licitacoes"
  ...
  if (path.startsWith("licitacoes.documentos."))
    return { resource: "licitacoes", action: "documents" as PermissionAction };
  ...
  const action: PermissionAction =
    leaf.includes("delete") || leaf.includes("excluir")
      ? "delete"
      : leaf.includes("create") ||
          leaf.includes("criar") ||
          leaf.includes("salvar")
        ? "create"
        : leaf.includes("update") ||
            leaf.includes("atualizar") ||
            ...
            leaf.includes("reset")
          ? "update"
          : leaf.includes("export")
            ? "export"
            : leaf.includes("import")
              ? "import"
              : leaf.includes("sincron")
                ? "sync"
                : "manage";
  return { resource, action };
}

export function legacyProfileEffect(
  profile: unknown,
  resource: string,
  action: string
): PermissionEffect {
  ...
  if (["comercial", "subcomercial"].includes(key)) {
    return ["inicio", "comercial", "estoque"].includes(resource)
      ? "allow"
      : "deny";
  }
  if (["gerencia", "diretoria"].includes(key))
    return resource === "usuarios" ? "deny" : "allow";
