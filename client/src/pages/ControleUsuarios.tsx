...
export default function ControleUsuarios() {
  ...
  const createUser = trpc.userManagement.create.useMutation({ onSuccess: async () => { toast.success("Usuário criado com sucesso."); setEditing(null); setForm(emptyForm); await users.refetch(); } });
  const updateUser = trpc.userManagement.update.useMutation({ onSuccess: async () => { toast.success("Usuário atualizado com sucesso."); setEditing(null); await users.refetch(); } });
