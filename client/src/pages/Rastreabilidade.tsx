...
export default function Rastreabilidade() {
  ...
  return <main className="trace-page">
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="trace-dialog"><DialogHeader><DialogTitle>Detalhes do evento #{selected?.id}</DialogTitle></DialogHeader>{selected && <div className="trace-detail">
      <div className="trace-detail-grid"><div><span>Data e hora</span><strong>{formatDate(selected.occurredAt)}</strong></div><div><span>Usuário</span><strong>{selected.displayName || selected.username || "Sistema"}</strong></div><div><span>Módulo</span><strong>{selected.module}</strong></div><div><span>Resultado</span><strong>{selected.result === "success" ? "Sucesso" : "Erro"}</strong></div><div><span>Procedimento</span><strong>{selected.procedurePath}</strong></div><div><span>IP</span><strong>{selected.ipAddress || "Não informado"}</strong></div></div>
